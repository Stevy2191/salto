...
- An **event** is just its name plus a shared/exclusive tag and color — it carries no duration; a **class owns its schedule** — its program, the day(s) it meets, its start time and period length, the events it's *eligible* for (each with the minutes this class spends there), and its warm-up/cool-down — all entered in one place, grouped by program, with a "copy setup from another class" shortcut. The same apparatus can run a different length for each class
