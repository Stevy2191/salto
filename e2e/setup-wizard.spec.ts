...
// One serial journey against a single fresh database: first-run wizard, the
// lane grid and drag-to-paint, generation + day-of repair, the print view,
// session copying, and the dark mode toggle.
...
test('first run: admin setup, then the setup wizard through to the grid', async ({ page }) => {
  ...
  await page.getByLabel('Event name').fill('Beam')
  await page.getByRole('button', { name: 'Save' }).click()
